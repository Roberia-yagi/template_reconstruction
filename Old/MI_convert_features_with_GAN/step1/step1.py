...
from util import (save_json, load_json, create_logger, resolve_path, load_model_as_feature_extractor, BatchLoader,
...
def get_options() -> Any:
    ...
    # Directories
    parser.add_argument("--result_dir", type=str, default="~/nas/results/MI_convert_features_with_GAN/step1", help="path to directory which includes results")
...
def loss_func(dists_A, dists_B):
    return torch.mean(torch.abs(dists_A - dists_B))
...
def main():
    ...
    for epoch in range(1, options.n_epochs + 1):
        ...
        for idx, (batch_dist, batch_data_and_idx) in enumerate(zip(batchloader_dist, dataloader_for_all)):
            ...
            dist_from_batch_to_average_A = calculate_dist_from_batch_to_average(batch_feature, averages_A, C)
            loss = loss_func(dist_from_batch_to_average_A, batch_dist)
