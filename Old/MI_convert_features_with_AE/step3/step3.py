...
from util import (save_json, load_json, create_logger, resolve_path, load_model_as_feature_extractor,
    load_model_as_feature_transfer, RandomBatchLoader, BatchLoader, load_attacker_discriminator,
...
def get_options() -> Any:
    ...
    parser.add_argument("--step1_dir", type=str, default="2022_04_18_13_59", help="path to directory which includes the step1 result")
    parser.add_argument("--model_dir", type=str, default="~/nas/results/common/pure_facenet_500epoch_features", help="path to directory which includes the step1 result")
...
def main():
    ...
    model_options = load_json(resolve_path(options.model_dir, "step2.json"))
    ...
    D = load_attacker_discriminator(
        path=resolve_path(options.model_dir, "D.pth"),
        input_dim=model_options["img_channels"],
        network_dim=model_options["D_network_dim"],
        img_shape=img_shape_T
    ).to(device)
    G = load_attacker_generator(
        path=resolve_path(options.model_dir, "G.pth"),
        latent_dim=options.latent_dim,
        network_dim=model_options["G_network_dim"],
        img_shape=img_shape_T
    ).to(device)
    ...
    C = load_model_as_feature_transfer(
        model=options.attack_model
    ).to(device)
