...
from util import (save_json, load_json, create_logger, resolve_path, load_model_as_feature_extractor, RandomBatchLoader,
    load_attacker_discriminator, load_attacker_generator, load_auto_encoder, get_freer_gpu, get_img_size)
...
def get_options() -> Any:
    ...
    # Directories
    parser.add_argument("--result_dir", type=str, default="~/nas/results/MI_convert_features_with_AE/step2_check_performance", help="path to directory which includes results")
    parser.add_argument("--step1_dir", type=str, default="2022_04_24_16_44", help="path to directory which includes results")
    parser.add_argument("--GAN_dir", type=str, default="~/nas/results/common/step2/pure_facenet_500epoch_features", help="path to directory which includes the step2 result")
...
def main():
    ...
    step1_dir = resolve_path("~/nas/results/MI_convert_features_with_AE/step1", options.step1_dir)
    ...
    for filename in glob.glob(target_imagefolder_path + "/*.jpg"):
        ...
        target_feature = T(converted_target_image.view(1, -1, img_size_T, img_size_T)).detach().to(device)
        all_target_features= torch.cat((all_target_features, target_feature.unsqueeze(0)))

        if options.single_mode:
            break

    all_target_features = AE(all_target_features)
    ...
    for i in range(1, options.times + 1):
        ...
        for epoch in range(1, options.epochs + 1):
            ...
            for batch_idx, batch in enumerate(dataloader):
                ...
                L_id_loss = calc_id_loss(G, A, batch, device, all_target_features, img_size_A)
        ...
        if options.save:
            ...
            for _, batch in enumerate(result_dataloader):
                ...
                best_image, _ = get_best_image(A, images, img_size_A, all_target_features)
            ...
            # Save the best image
            best_image, best_cosine_similarity = get_best_image(A, all_best_images, img_size_A, all_target_features)
