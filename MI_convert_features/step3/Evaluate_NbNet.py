...
def get_options() -> Any:
    ...
    parser.add_argument("--NbNet_dir", type=str, required=True)
...
def main():
    ...
    for identity_path in identity_paths:
        ...
        if folder_name == 'best_images':
            continue
        filename_paths = glob.glob(resolve_path(identity_path, '*')) # /.../Aaron_Guiel/Aaron_Guiel0001.jpg
        for filename_path in filename_paths:
            # Extract a feature from the reconstructed image
            file_name = remove_path_prefix(filename_path) # Aaron_Guiel0001.jpg
            NbNet_reconstrected_image_path = resolve_path(options.NbNet_dir, folder_name, file_name) # /.../Aaron_Guiel/Aaron_Guiel0001.jpg
            NbNet_reconstructed_image = transform_T(PIL.Image.open(NbNet_reconstrected_image_path))
            NbNet_aligned_reconstructed_image = align_face_image(NbNet_reconstructed_image, 'GAN', step1_options['target_model'], detector)
            if NbNet_aligned_reconstructed_image is None:
                logger.info('the reconstructed image cannot be aligned')
            reconstructed_feature = T(NbNet_aligned_reconstructed_image.to(device)).unsqueeze(0)
    ...
    # logger.info(f'Total comparison: {len(cossims_TypeA)}')
    ...
    # logger.info(f'Total comparison: {len(cossims_TypeB)}')
